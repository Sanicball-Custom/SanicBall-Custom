    ...
    public class Server : IDisposable
    {
        ...
        public Server(CommandQueue commandQueue, bool local)
        {
            ...
            cmd =>
            {
                ...
                {
                    Log("Usage: setStage [0-" + (STAGE_COUNT - 1) + "] / setStage [StageName / StageAlias]");
                }
            });
            AddCommandHandler("alias",
            ...
            cmd =>
            {
                ...
                int stageInt;
                int stageAlias; // Only used if the stage name we are aliasing is itself an alias.
                if (int.TryParse(stage, out stageInt) && stageInt >= 0 && stageInt < STAGE_COUNT)
                {
                    matchSettings.Aliases.Add(alias, stageInt);
                    SaveMatchSettings();
                    SendToAll(new SettingsChangedMessage(matchSettings));
                    Log("Aliased " + stageInt + " with " + alias);
                }
                else if(cmd.Content is string)
                {
                    if(Enum.IsDefined(typeof(Tracks), stage))
                    {
                        ...
                        matchSettings.Aliases.Add(alias, stageAlias);
                        SaveMatchSettings();
                        Log("Aliased " + stage + " with " + alias);
                    }else{
                        string bestMatch="";
                ...
                {
                    Log("Usage: alias [0-" + (STAGE_COUNT - 1) + " / StageName / StageAlias] [AliasName]");
                }
            });
            AddCommandHandler("rmalias",
            "Removes an alias for a stage (no spaces).",	
            cmd =>
            {
                if(cmd.Content.Length > 0){
                    if(matchSettings.Aliases.ContainsKey(cmd.Content)){
                        matchSettings.Aliases.Remove(cmd.Content);
                        SaveMatchSettings();
                        Log("Removed alias " + cmd.Content);
                    }else{
                        string bestMatch="";
                        double maxScore=0;
                        ...
                        Log("Alias " + cmd.Content + " doesn't exist, were you looking for " + bestMatch);
                    }
                }
                else
                {
                    Log("Usage: rmalias [StageAlias]");
                }
            });
            AddCommandHandler("lsalias",
            "Lists all aliases.",	
            cmd =>
            {
                foreach(string name in matchSettings.Aliases.Keys){
            ...
            "Sets a player as admin, only one admin can be set at once.",
            cmd =>
            {
                if(cmd.Content.Length > 0){
                    SetAdmin(cmd.Content);
                }
                else
                {
            ...
            if (!File.Exists(CONFIG_FILENAME) && !isLocalServer)
            {
        ...
        public void Start(int port, int maxPlayers, string name, string admin, string matchSettingsPath)
        {
            ...
            if(!isLocalServer){
                ...
                if (!LoadMatchSettings(matchSettingsPath))
                    matchSettings = MatchSettings.CreateDefault();
            }
        ...
        public void SetAdmin(string name){
            ServClient client = clients.FirstOrDefault(a => a.Name == name);
            if(client != null){
                adminGuid = client.Guid;
            }else{
                adminName = name;
            }
            Whisper(client, "You are now THE admin.");
            Log(client.Name + " is now the admin.");
        }
                            ...
                            switch (status)
                            {
                                ...
                                case NetConnectionStatus.Disconnected:
                                    ...
                                    if (associatedClient != null)
                                    {
                                        ...
                                        if(isLocalServer && associatedClient.Name == ownerName){
                                            running = false;
                                        }
                            ...
                            switch (messageType)
                            {
                                ...
                                case MessageType.MatchMessage:
                                    ...
                                    if (matchMessage is ChatMessage)
                                    {
                                        ...
                                        ServClient client = clients.FirstOrDefault(a => a.Connection == msg.SenderConnection);
                                        if(client.Guid == adminGuid && castedMsg.Text.StartsWith("/")){
                                            if(commandHandlers.ContainsKey(castedMsg.Text.Substring(1,castedMsg.Text.IndexOf(' ')).Trim())){
                                                commandQueue.Add(new Command(castedMsg.Text.Substring(1)));
                                            }else{
                                                Whisper(client, "Unknown command: " + castedMsg.Text.Substring(1,castedMsg.Text.IndexOf(' ')));
                                            }
                                        }else{
                                    ...
                                    {
                                        var castedMsg = (PlayerForcedRespawnMessage)matchMessage;
                                        ServPlayer player = players.FirstOrDefault(a => a.ClientGuid == castedMsg.ClientGuid && a.CtrlType == castedMsg.CtrlType);
                                        if (player != null)
                                        {
                                            SendToAll(castedMsg);
