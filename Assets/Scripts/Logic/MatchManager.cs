...
namespace Sanicball.Logic
{
    ...
    public class MatchManager : MonoBehaviour
    {
		...
		private void PlayerZappedCallback(PlayerZappedMessage msg, float travelTime) {
            var player = players.FirstOrDefault(a => a.ClientGuid == msg.ClientGuid && a.CtrlType == msg.CtrlType);
            if (player != null) {
                Debug.Log("Player zapped (MatchManager): " + player.BallObject.Nickname);
				player.BallObject.zapped = true;
			}
        }

        private void PlayerConfusedCallback(PlayerConfusedMessage msg, float travelTime) {
            var player = players.FirstOrDefault(a => a.ClientGuid == msg.ClientGuid && a.CtrlType == msg.CtrlType);
            if (player != null) {
                Debug.Log("Player confused (MatchManager): " + player.BallObject.Nickname);
				player.BallObject.confused = true;
			}
        }
        ...
        private void Start()
        {
            ...
            messenger.CreateListener<PlayerZappedMessage>(PlayerZappedCallback);
            messenger.CreateListener<PlayerConfusedMessage>(PlayerConfusedCallback);
            messenger.CreateListener<PlayerForcedRespawnMessage>(PlayerForcedRespawnCallback);
        ...
        private void SpawnLobbyBall(MatchPlayer player)
        {
			...
			if(player.ClientGuid == myGuid){
				Sanicball.Extra.DiscordUtils.UpdateActivity(ActiveData.Characters[player.CharacterId].name);
			}
