...
namespace Sanicball.Logic
{
    ...
    public class RaceManager : MonoBehaviour
    {
        ...
        public RaceState RaceState { get { return currentState; } }
        ...
        private void CreateBallObjects()
        {
            ...
            for (int i = 0; i < matchManager.Players.Count; i++)
            {
                ...
                //Integrate with Discord & Character Specific Music
                if (local)
                {
        ...
        {
            if (disqualified) rp.FinishRace(new RaceFinishReport(rp.Position, System.TimeSpan.FromSeconds(raceTime + 1000)));
            else rp.FinishRace(new RaceFinishReport(rp.Position, System.TimeSpan.FromSeconds(raceTime)));
        ...
        private void Update()
        {
            ...
            //Fix wrong positions when finished race
            int finishIndex = 0;
            foreach (RacePlayer rp in players.FindAll(a => a.RaceFinished).OrderBy(a => a.FinishReport.Time))
            {
                rp.Position = finishIndex + 1;
                rp.FinishReport.SetPosition(finishIndex + 1);
                finishIndex++;
            }

            //Order player list by position (without moving those who finished)
            foreach (RacePlayer rp in players.OrderByDescending(a => a.CalculateRaceProgress()))
            {
                if (!rp.RaceFinished)
                {
                    rp.Position = finishIndex + 1;
            ...
            players = players.OrderBy(a => a.Position).ToList();
        }
